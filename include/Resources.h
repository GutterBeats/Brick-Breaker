//
// Created by Anthony Lesch on 7/25/24.
//

#pragma once

//
// Textures
//
// IMPR: This can updated to scan the resources folder for a specific pattern and load them dynamically.
//
#define BLUE_BRICK_TEXTURE "resources/Textures/element_blue_rectangle_glossy.png"
#define GREEN_BRICK_TEXTURE "resources/Textures/element_green_rectangle_glossy.png"
#define GREY_BRICK_TEXTURE "resources/Textures/element_grey_rectangle_glossy.png"
#define PURPLE_BRICK_TEXTURE "resources/Textures/element_purple_rectangle_glossy.png"
#define RED_BRICK_TEXTURE "resources/Textures/element_red_rectangle_glossy.png"
#define PLAYER_RED_TEXTURE "resources/Textures/paddleRed.png"
#define BALL_BLUE_TEXTURE "resources/Textures/ballBlue.png"
#define PAUSE_TEXTURE "resources/Textures/PauseTexture.png"
#define BUTTON_TEXTURE "resources/Textures/buttonDefault.png"
#define SELECTED_BUTTON_TEXTURE "resources/Textures/buttonSelected.png"

//
// Images
//
#define BB_BACKGROUND_ALPHA 0.35f
#define COMPANY_IMAGE "resources/Images/PeeperGames.jpeg"
#define ENGINE_IMAGE "resources/Images/LazloEngine.jpeg"
#define BACKGROUND_IMAGE "resources/Images/Background.jpeg"
#define MENU_BACKGROUND_IMAGE "resources/Textures/MenuBackground.png"

//
// Fonts
//
#define DEFAULT_FONT "resources/Fonts/Kenney Pixel Square.ttf"

//
// Sounds
//
#define PADDLE_COLLISION_SFX "resources/Sounds/BrickCollision.wav"
#define BRICK_COLLISION_SFX "resources/Sounds/PaddleCollision.wav"

//
// Music
//
#define MAIN_MUSIC "resources/Music/MainMusic.wav"
#define GAMEOVER_MUSIC "resources/Music/GameOverMusic.wav"
